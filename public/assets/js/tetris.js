const Page = {
    ...
    WindowChanged: function () {
        ...
        this.unitSize = Math.max(Math.min(newUnitW, newUnitH), 20);
        ...
        for (let i = 0; i < Page.AreaArr.length; i++) {
            ...
            const newRightLimit = Page.AreaArr[i].left + Page.AreaArr[i].W,
...
Page.Game = new DrawAreaObj(0,0,10,20,function(){
  ...
  for(let i = 0; i < GM.StaticUnits.length; i++){
    for(let j = 0; j < GM.StaticUnits[i].length; j++){
      ...
      // get the unit value for this index pair
        const uValue = GM.StaticUnits[i][j];
  ...
  if (GM.Pc.Cur !== 0 && GM.IsAlive){
      const projColor = ColorWithAlpha(GM.Pc.Cur.color, 0.1);
  ...
  if (!GM.IsAlive){
    DrawText(`Puntuación: ${GM.ScoreHigh}`,'rgb(255,255,255)','500',
...
Page.UpcomingA = new DrawAreaObj(10.5,2.6,2.5,2.5,function(){
    ...
    const uDrawSize = Math.floor(Page.unitSize / 2),
  ...
  if (pcA !== 0){
    ...
    for(let i = 0; i < pcA.UO.arr.length; i++){
      ...
      const avgL = uDrawSize * (totalL / countedL.length + 0.5),
    ...
    for(let j = 0; j < pcA.UO.arr.length; j++){
        const drawL = Math.floor(offsetL - avgL + pcA.UO.arr[j].x * uDrawSize),
...
Page.UpcomingB = new DrawAreaObj(10.5,5.2,2.5,2.5,function(){
  ...
  if (pcB !== 0){
    ...
    for(let j = 0; j < pcB.UO.arr.length; j++){
        const drawL = Math.floor(offsetL - avgL + pcB.UO.arr[j].x * uDrawSize),
...
Page.UpcomingC = new DrawAreaObj(10.5,7.8,2.5,2.5,function(){
  ...
  if (pcC !== 0){
    ...
    for(let j = 0; j < pcC.UO.arr.length; j++){
        const drawL = Math.floor(offsetL - avgL + pcC.UO.arr[j].x * uDrawSize),
...
Page.ScoreBarHigh = new DrawAreaObj(10.5,0,4.5,1,function(){
  ...
  // draw the player's current score
    let text = ("00000000" + GM.ScoreHigh).slice(-7);
  left = this.left + this.W - 4;
  top = this.top + Page.unitSize * 0.8;
    let size = Math.floor(Page.unitSize * 0.8) + 0.5;
...
GM.Pc = {
  ...
  Freeze: function(){
    ...
    if (GM.IsAlive){
        ...
        for(let i = 0; i < this.Cur.UO.arr.length; i++){
            const staticX = this.Cur.x + this.Cur.UO.arr[i].x,
  ...
  DoGravity: function(){
    if (this.Cur !== 0){
        const collisions = this.CheckCollisions(0, 0, 1);
  ...
  TryRotate: function(){
    if (this.Cur !== 0){
        const collisions = this.CheckCollisions(1, 0, 0);
  ...
  TryMove: function(moveX, moveY){
    if (this.Cur !== 0){
        const collisions = this.CheckCollisions(0, moveX, moveY);
  ...
  CheckCollisions: function(doRot, offsetX, offsetY){
    ...
    for(let i = 0; i < unitArr.length; i++){
        const testX = this.Cur.x + unitArr[i].x + offsetX,
